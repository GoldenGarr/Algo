...
public class InversionsGodHelpMe {
    ...
    public static void postMergeSort(int[] array) {
        if (array.length <= 1) {
            return;
        } else {
    ...
    public void run() {
        try {
            br = new BufferedReader(new FileReader("inversions.in"));
            out = new PrintWriter("inversions.out");
            solve();
            out.close();
        } catch (IOException e) {
            e.printStackTrace();
